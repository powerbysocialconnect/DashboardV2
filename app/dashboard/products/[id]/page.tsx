...
import { ArrowLeft, Plus, X, Loader2, Trash2, PlusCircle, GripVertical, Image as ImageIcon, CheckCircle2, AlertCircle, Sparkles } from "lucide-react";
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const optionGroups = (prod.product_option_groups || [])
      ...
      // Map relational variants
      const variants = (prod.product_variants || []).map((v: any) => {
        const optionValues: Record<string, string> = {};
        (v.product_variant_options || []).forEach((vo: any) => {
          const groupName = optionGroups.find(
            (og: any) => og.id === vo.product_option_values.option_group_id
          )?.name;
          if (groupName) {
            optionValues[groupName] = vo.product_option_values.value;
          }
        });
  ...
  const generateVariants = () => {
    ...
    // Filter out groups without values
    ...
    function cartesian(groupIndex: number, currentCombination: Record<string, string>) {
      ...
      group.values.forEach((val) => {
        currentCombination[group.name] = val.value;
    ...
    const newVariants = combinations.map((combo) => {
      // Try to find existing variant with same combo to preserve its data
    ...
    try {
      ...
      if (values.category_ids?.length > 0) {
        await supabase.from("product_categories").insert(
          values.category_ids.map(catId => ({ product_id: product.id, category_id: catId }))
        );
      }

      // 3. Update Options and Variants (Relational)
      // For simplicity in this implementation, we'll clear and re-insert.
      // In a production app, you might want to sync instead of wipe to preserve IDs.
      
      // Clear existing options (cascades to product_option_values)
      await supabase.from("product_option_groups").delete().eq("product_id", product.id);
      // Clear existing variants (cascades to product_variant_options)
      await supabase.from("product_variants").delete().eq("product_id", product.id);

      if (values.option_groups.length > 0) {
        // Insert Option Groups and Values
        for (let i = 0; i < values.option_groups.length; i++) {
          ...
          const { data: valuesData, error: ovError } = await supabase
          ...
          // We'll need these value IDs to link variants later
          // Store them in a helper map
        }

        // Re-fetch option values to get their IDs for variant mapping
